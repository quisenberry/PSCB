'''
This config holds all user adjustable vars. Vars should not be changed directly on the Pi but in the GIT repo then
pushed and auto updated. Changing vars here will break the auto update system.


Author: Ryan Mills <ryan@flexhibit.com>
Copyright (c) 2018 Flexhibit - All Rights Reserved

THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR
IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY,
FITNESS FOR A PARTICULAR PURPOSE AND NONINFRINGEMENT. IN NO EVENT SHALL THE
AUTHORS OR COPYRIGHT HOLDERS BE LIABLE FOR ANY CLAIM, DAMAGES OR OTHER
LIABILITY, WHETHER IN AN ACTION OF CONTRACT, TORT OR OTHERWISE, ARISING FROM,
OUT OF OR IN CONNECTION WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS IN
THE SOFTWARE.
'''

import os
import sys

# DEPLOY VARS
GIT_POLL_RATE = 5 #time in seconds the script should check for GIT updates
GIT_REPO = 'https://github.com/quisenberry/PSCB.git'
PATH_ABSOLUT = os.path.dirname(os.path.abspath(sys.argv[0]))
DEPLOY_PATH = os.path.dirname(os.path.abspath(sys.argv[0]))+os.path.sep+'exhibit.py'
CRASH_PATH = os.path.dirname(os.path.abspath(sys.argv[0]))+os.path.sep+'logs'+os.path.sep


# MAIN
PSCB_NAME = 'PSCB'
PSCB_VERISON = '0.0.2'

# TIMING
RESET_TIMEOUT = 120 #time in seconds

# SOUNDS
SOUNDS_PATH = PATH_ABSOLUT+os.path.sep+'sounds'+os.sep
SOUNDS_SIREN = SOUNDS_PATH+'siren.wav'
SOUNDS_HORN = SOUNDS_PATH+'horn.wav'
SOUNDS_CRASH = SOUNDS_PATH+'crash.wav'
SOUNDS_CROSSWALK = SOUNDS_PATH+'crosswalk.wav'
SOUNDS_EXTRA = SOUNDS_PATH+'extra.wav'
SOUNDS_COMPLETE = SOUNDS_PATH+'complete.wav'
SOUNDS = [SOUNDS_SIREN, SOUNDS_HORN, SOUNDS_CRASH, SOUNDS_CROSSWALK, SOUNDS_EXTRA, SOUNDS_COMPLETE]


# PIN MAPPING
INPUT_TRAIN = 2
INPUT_CROSSING = 3
INPUT_SIGNAL = 4
INPUT_EME = 5
INPUT_CROSSWALK = 6
INPUT_EXTRA = 7
INPUT_MODE = 24

PWR_TRAIN = 8
PWR_CROSSING = 9

LED_MODE1 = 10
LED_MODE2 = 11
LED_MODE3 = 12
LED_SIGNAL_GREEN = 13
LED_SIGNAL_YELLOW = 14
LED_SIGNAL_RED = 15
LED_TRAIN = 16
LED_SIGNAL = 17
LED_CROSSWALK = 18
LED_EME = 19
LED_CROSSING = 20
EMPTY_OUT1 = 21
EMPTY_OUT2 = 22
EMPTY_OUT3 = 23

PIN_GROUP_INPUT = [INPUT_TRAIN, INPUT_CROSSING, INPUT_SIGNAL, INPUT_EME, INPUT_CROSSWALK, INPUT_EXTRA, INPUT_MODE]
PIN_GROUP_INPUT_FILTER = [INPUT_TRAIN, INPUT_CROSSING, INPUT_SIGNAL, INPUT_EME, INPUT_CROSSWALK, INPUT_EXTRA]
PIN_GROUP_OUTPUT = [
    PWR_TRAIN,
    PWR_CROSSING,
    LED_MODE1,
    LED_MODE2,
    LED_MODE3,
    LED_TRAIN,
    LED_SIGNAL,
    LED_SIGNAL_GREEN,
    LED_SIGNAL_YELLOW,
    LED_SIGNAL_RED,
    LED_CROSSWALK,
    LED_EME,
    LED_CROSSING,
    EMPTY_OUT1,
    EMPTY_OUT2,
    EMPTY_OUT3
]

PIN_GROUP_OUTPUT_LEDS = [
    LED_MODE1,
    LED_MODE2,
    LED_MODE3,
    LED_TRAIN,
    LED_SIGNAL,
    LED_SIGNAL_GREEN,
    LED_SIGNAL_YELLOW,
    LED_SIGNAL_RED,
    LED_CROSSWALK,
    LED_EME,
    LED_CROSSING,
    EMPTY_OUT1,
    EMPTY_OUT2,
    EMPTY_OUT3
]
PIN_GROUP_MODE = [LED_MODE1, LED_MODE2, LED_MODE3]

PIN_RELAY_BOARD_START = 8



